...
//add for camera info begin by koson 20171218
#include "../lct_camera_info.h"
//add for camera info end by koson 20171218

#define PFX "HI846_camera_sensor"
//#define LOG_WRN(format, args...) xlog_printk(ANDROID_LOG_WARN ,PFX, "[%S] " format, __FUNCTION__, ##args)
//#defineLOG_INF(format, args...) xlog_printk(ANDROID_LOG_INFO ,PFX, "[%s] " format, __FUNCTION__, ##args)
//#define LOG_DBG(format, args...) xlog_printk(ANDROID_LOG_DEBUG ,PFX, "[%S] " format, __FUNCTION__, ##args)
#define LOG_INF(format, args...)	pr_debug(PFX "hi846:[%s][%d] " format, __FUNCTION__, __LINE__, ##args)
...
static imgsensor_info_struct imgsensor_info = {
    ...
    .pre = {
        .pclk = 287800000,				//record different mode's pclk
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] =
{
    ...
    { 3280, 2464,   348, 504, 2568, 1440,   1284,  720,   2, 0, 1280,  720,   0, 0, 1280, 720 }
};// slim video
...
static int HI846_otp_read(void)
{
    ...
    if (addr != 0) {
        ...
        HI846_otp.AF_FLAG = OTP_read_cmos_sensor(addr + 1);
        ...
        HI846_otp.sensor_id = OTP_read_cmos_sensor(addr + 5);
        HI846_otp.lens_id = OTP_read_cmos_sensor(addr + 6);
        HI846_otp.vcm_id = OTP_read_cmos_sensor(addr + 7);
        HI846_otp.Driver_ic_id = OTP_read_cmos_sensor(addr + 8);
        HI846_otp.infocheck = OTP_read_cmos_sensor(addr + 16);

        checksum = (HI846_otp.module_integrator_id + HI846_otp.AF_FLAG + HI846_otp.prodyction_year + HI846_otp.production_month + HI846_otp.production_day + HI846_otp.sensor_id + HI846_otp.lens_id + HI846_otp.vcm_id + HI846_otp.Driver_ic_id) % 0xFF + 1;

        LOG_INF("chceksum=0x%x  infocheck=0x%x\n", checksum, HI846_otp.infocheck);
        ...
        LOG_INF("%s:%d: Hi846 module info data invalid!\n", __func__, __LINE__);
        return -1;
    }
    ...
    if (wb_start_addr != 0) {
        ...
        for (i = 0; i < 30; i++) {
            ...
            //LOG_INF("HI846 wb_data[%d] = 0x%x\n", i, HI846_otp.wb_data[i]);
            LOG_INF("reg[0x%x] = 0x%x\n", wb_start_addr + i, HI846_otp.wb_data[i]);
        }
        ...
        LOG_INF("chceksum=0x%x  wbcheck=0x%x\n", checksum, HI846_otp.wbcheck);
        if (checksum == HI846_otp.wbcheck)
        {
        ...
        LOG_INF("%s:%d: Hi846 WB data invalid!\n", __func__, __LINE__);
        return -1;
    }

    HI846_otp.AF_FLAG = OTP_read_cmos_sensor(0x0CBA);
    LOG_INF("HI846 AF_FLAG = 0x%x\n",HI846_otp.AF_FLAG);
    if (HI846_otp.AF_FLAG == 0x01)
        af_start_addr = 0x0CBB;
    else if (HI846_otp.AF_FLAG == 0x13)
        af_start_addr = 0x0CC1;  //zy
    else if (HI846_otp.AF_FLAG == 0x37)
        af_start_addr = 0x0CC7;  //zy
    else
        af_start_addr = 0;
    ...
    if (af_start_addr != 0) {
        ...
        for (i = 0; i < 6; i++) {
            ...
            LOG_INF("HI846 af_data[%d] = 0x%x\n", i, HI846_otp.af_data[i]);
        }

        checksum = 0;
        for (i = 0; i < 5; i++) {
            checksum += HI846_otp.af_data[i];
        }
        checksum = (checksum % 0xff) + 1;
        HI846_otp.afcheck = HI846_otp.af_data[i];

        LOG_INF("chceksum=0x%x  afcheck=0x%x\n", checksum, HI846_otp.afcheck);
        ...
        LOG_INF("%s:%d: Hi846 AF data invalid!\n", __func__, __LINE__);
        return -1;
    }

    spin_lock(&imgsensor_drv_lock);
    hi846_otp_data[0] = 0xff;  //zy layout check
    hi846_otp_data[1] = 0x00;  //zy layout check
    hi846_otp_data[2] = 0x0b;  //zy layout check
    hi846_otp_data[3] = 0x1;  //zy layout check

    hi846_otp_data[4] = 0x2;  //bit1:awb bit1:af bit4:lsc zy  choose mode
    hi846_otp_data[5] = 0;	//reserved

    hi846_otp_data[6] = HI846_otp.af_data[1];//Infinity_L;
    hi846_otp_data[7] = HI846_otp.af_data[0];//Infinity_H;
    hi846_otp_data[8] = HI846_otp.af_data[3];//Near_L;
    hi846_otp_data[9] = HI846_otp.af_data[2];//Near_H;
    spin_unlock(&imgsensor_drv_lock);
...
static void HI846_otp_cali(void)
{
    ...
    if (!HI846_otp_read()) {
        HI846_otp_apply();
        lct_camera_set_proc_otpinfo_flag(0, 1);
    } else {
        printk(KERN_ERR "%s:%d: Hi846 otp data read error!\n", __func__, __LINE__);
    }
}
#endif
...
static void sensor_init(void)
{
    ...
    write_cmos_sensor(0x0F38, 0x0253);
...
//add for camera info begin by koson 20171218
    lct_camera_set_proc_camera_name(0, "Holitech", "Hi846");
//add for camera info end by koson 20171218
