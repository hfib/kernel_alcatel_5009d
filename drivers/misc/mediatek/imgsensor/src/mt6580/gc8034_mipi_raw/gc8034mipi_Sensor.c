...
#define LOG_INF(format, args...)    printk(PFX "[%s][%d] " format, __func__, __LINE__, ##args)
...
static int otp_info_check_flag;
static int otp_awb_check_flag;
static int otp_af_check_flag;
#endif
...
static void gc8034_gcore_update_otp(void)
{
    ...
    if ((1 == otp_info_check_flag) && (1 == otp_awb_check_flag) && (1 == otp_af_check_flag)) {
        gc8034_gcore_update_wb();
        gc8034_gcore_update_lsc();
        lct_camera_set_proc_otpinfo_flag(0, 1);
    } else {
        printk(KERN_ERR "%s:%d: info or awb or af otp data check failed! otp_info_check_flag=%d otp_awb_check_flag=%d otp_af_flag=%d\n", __func__, __LINE__, otp_info_check_flag, otp_awb_check_flag, otp_af_check_flag);
    }
...
static kal_uint32 open(void)
{
    ...
    lct_camera_set_proc_camera_name(0, "TSP", "GC8034");
