...
#define PFX "hi556"
#define LOG_INF(format, args...)    pr_err(PFX "hi556:[%s][%d] " format, __FUNCTION__, __LINE__, ##args)
#define LOG_DEBUG(format, args...)    pr_err(PFX "hi556:[%s][%d] " format, __FUNCTION__, __LINE__, ##args)
...
//module info read
    ...
    if (info_start_addr != 0) {
        ...
        info_check = HI556_otp.info_data[16] & 0xff;
        ...
        LOG_INF("%s:%d: Hi556 module info data invalid!\n", __func__, __LINE__);
        return -1;
    }
...
//wb read
    ...
    if (wb_start_addr != 0) {
        ...
        checksum = (checksum % 0xff) + 1;
        wb_check =  HI556_otp.wb_data[29]&0xff;
        ...
        LOG_INF("%s:%d: HI556 WB data invalid!\n", __func__, __LINE__);
        return -1;
    }
...
static int HI556_otp_apply(void)
{
    ...
    LOG_DEBUG("RG_ratio_golden=0x%x,BG_ratio_golden=0x%x\n",RG_ratio_golden,BG_ratio_golden);
    ...
    if (R_gain < B_gain) {
        if(R_gain < 0x100) {
            ...
            R_gain = 0x100;
            LOG_DEBUG("HI556_otp_apply-789-R_gain=%d,B_gain=%d\n",R_gain,B_gain);
        }
    } else {
        if (B_gain < 0x100) {
            ...
            B_gain = 0x100;
            LOG_DEBUG("HI556_otp_apply-796-R_gain=%d,B_gain=%d\n",R_gain,B_gain);
        }
    }
...
static int hi556_otp_lsc_read(void)
{
    ...
    if (lsc_start_addr != 0) {
        ...
        checksum_lsc = (checksum_lsc - hi556_otp.lsc_data[1868])%255+1;
...
static void HI556_otp_cali(void)
{
    ...
    if (!HI556_otp_read()) {
        HI556_otp_apply();
        lct_camera_set_proc_otpinfo_flag(1, 1);
    } else {
        printk(KERN_ERR "%s:%d: Hi556 otp data read error!\n", __func__, __LINE__);
    }
}
#endif
...
//add for camera info begin by koson 20171218
    lct_camera_set_proc_camera_name(1, "Holitech", "Hi556");
//add for camera info end by koson 20171218
