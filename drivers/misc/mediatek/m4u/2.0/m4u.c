...
static long MTK_M4U_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case MTK_M4U_T_ALLOC_MVA:
		...
		if (m4u_module.port < M4U_PORT_0 || m4u_module.port >= M4U_PORT_NR) {
			M4UMSG("invalid port id[%d] copied from user space!\n", m4u_module.port);
			return -EINVAL;
		}
	...
	case MTK_M4U_T_CACHE_SYNC:
		...
		if (m4u_cache_data.port < M4U_PORT_0 || m4u_cache_data.port >= M4U_PORT_NR) {
			M4UMSG("invalid port id[%d] copied from user space!\n", m4u_cache_data.port);
			return -EINVAL;
		}
	...
	case MTK_M4U_T_DMA_OP:
		...
		if (m4u_dma_data.port < M4U_PORT_0 || m4u_dma_data.port >= M4U_PORT_NR) {
			M4UMSG("invalid port id[%d] copied from user space!\n", m4u_dma_data.port);
			return -EINVAL;
		}
	...
	case MTK_M4U_T_CONFIG_PORT:
		...
		if (m4u_port.ePortID < M4U_PORT_0 || m4u_port.ePortID >= M4U_PORT_NR) {
			M4UMSG("invalid port id[%d] copied from user space!\n", m4u_port.ePortID);
			return -EINVAL;
		}
#ifdef M4U_TEE_SERVICE_ENABLE
	...
	case MTK_M4U_T_MONITOR_START:
		...
		if (PortID < M4U_PORT_0 || PortID >= M4U_PORT_NR) {
			M4UMSG("invalid port id[%d] copied from user space!\n", PortID);
			return -EINVAL;
		}
	...
	case MTK_M4U_T_CONFIG_MAU:
		{
			...
			if (rMAU.port < M4U_PORT_0 || rMAU.port >= M4U_PORT_NR) {
				M4UMSG("invalid port id[%d] copied from user space!\n", rMAU.port);
				return -EINVAL;
			}
			ret = config_mau(rMAU);
	...
	case MTK_M4U_T_CONFIG_TF:
		{
			...
			if (rM4UTF.port < M4U_PORT_0 || rM4UTF.port >= M4U_PORT_NR) {
				M4UMSG("invalid port id[%d] copied from user space!\n", rM4UTF.port);
				return -EINVAL;
			}
