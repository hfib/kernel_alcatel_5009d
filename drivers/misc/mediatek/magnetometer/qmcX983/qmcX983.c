...
{	
	...
	if ((txData == NULL) || (length < 2))
		return -EINVAL;
	res = mag_i2c_write_block(client, addr, buf, (length - 1));
	if (res < 0)
		return -1;
	return 0;
...
static ssize_t store_registers_addr(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if(1 == rebuf[0]){//sscanf(buf,"0x%2x",(unsigned char*)&regbuf[0])
...
static int qmcX983_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	this_client = new_client;
	 ...
	 MSE_LOG("qmcX983 i2c probe 2\n");
	   ...
	   databuf[0] = 0x18;
	   databuf[0] = 0x80;
       dummy = I2C_TxData(databuf,2);;//send_data(client, 0x18, 0x80);
	 //  if (dummy < 0)
		//  return -1;
