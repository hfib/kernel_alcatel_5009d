...
//Begin xiaopei add LCM info 
 char lcm_ver_show_str[64];
//End xiaopei add LCM info 12-14
		...
		if (plcm_name == NULL) {
			...
			for (i = 0; i < _lcm_count(); i++) {
				...
				if (!strcmp(lcm_drv->name, plcm_name)) {
					...
					lcmindex = i;
					//Begin xiaopei add LCM info  
					memset(lcm_ver_show_str, 0, sizeof(lcm_ver_show_str));
					sprintf(lcm_ver_show_str, "%s",lcm_drv->name);
					//init_tp_fm_info(0,lcm_ver_show_str,"Ilitek");
					//end add 12-14
					break;
				}
			}
