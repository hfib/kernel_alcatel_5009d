...
static int mtkfb_set_par(struct fb_info *fbi)
{
	...
	/* workaround for second lk logo showing from gralloc */
	/* after SF first drawing, no_update will be set false, defalut is true */
	if (no_update == false)
		primary_display_config_input_multiple(session_input);
	else
		DISPMSG("mtkfb_set_par will jump first display config, no_update=%d, before HWC works.\n", no_update);
	kfree(session_input);
