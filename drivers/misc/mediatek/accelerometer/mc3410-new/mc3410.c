...
//start change by ldy 20180102
extern bool is_Vibr_Enable(void);
//end change by ldy 20180102
...
//start change by ldy 20180102
static int16_t pre_data[MC3XXX_AXES_NUM] = {0};
static int16_t Vibcount=0;
static void MC3XXX_LPF(s16 data[MC3XXX_AXES_NUM])
{
	int16_t filter,i;

	if(pre_data[MC3XXX_AXIS_X] ==0&&pre_data[MC3XXX_AXIS_Y]==0&&pre_data[MC3XXX_AXIS_Z]==0  )
		{
			pre_data[MC3XXX_AXIS_X] = data[MC3XXX_AXIS_X];
			pre_data[MC3XXX_AXIS_Y] = data[MC3XXX_AXIS_Y];
			pre_data[MC3XXX_AXIS_Z] = data[MC3XXX_AXIS_Z];

		}
    	if(is_Vibr_Enable())  //  vibrator_state 
    		{
	    		filter=50;
				Vibcount =10; // when motor stop , ten additional data will be handled . You can changed it.
			}
		else
		{
			if(Vibcount>0)
	...
	for(i=0;i<3;i++){
		if(((data[i] - pre_data[i]) >filter) || ((pre_data[i] - data[i]) > filter))
			data[i] =((pre_data[i] - data[i])>0?(pre_data[i] - filter):(pre_data[i] + filter));
	}
...
static int	MC3XXX_ReadData(struct i2c_client *pt_i2c_client, s16 waData[MC3XXX_AXES_NUM])
{
	...
	if (!s_nIsRBM_Enabled) {
		...
		//MC3XXX_LPF(_ptPrivData, waData);
			...
			printk("LPF<<<<<[%06d %06d %06d]\n", waData[MC3XXX_AXIS_X], waData[MC3XXX_AXIS_Y],
				waData[MC3XXX_AXIS_Z]);
		
		MC3XXX_LPF(waData);
		pre_data[MC3XXX_AXIS_X] = waData[MC3XXX_AXIS_X];
		pre_data[MC3XXX_AXIS_Y] = waData[MC3XXX_AXIS_Y];
		pre_data[MC3XXX_AXIS_Z] = waData[MC3XXX_AXIS_Z];
		
			printk("LPF<<<<<[%06d %06d %06d]\n", pre_data[MC3XXX_AXIS_X], pre_data[MC3XXX_AXIS_Y],
				pre_data[MC3XXX_AXIS_Z]);
...
static int mc3xxx_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	init_sensor_version_info(ACCELEROMETER_NO , "MC3410" , "gsensor");
#endif
	return 0;
