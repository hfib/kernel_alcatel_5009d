...
/*BEGIIN: modified by zhufeng 20171225 for multi battery support*/
#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
typedef signed int		kal_int32;	

kal_int32 g_Q_MAX_POS_50[] = {2438, 2427};
kal_int32 g_Q_MAX_POS_25[] = {2371, 2443}; //2421 modified by zhufeng 20180131 for discharging too fast 
kal_int32 g_Q_MAX_POS_0[] = {2355, 2434};  //2405 modified by zhufeng 20180131 for discharging too fast 
kal_int32 g_Q_MAX_NEG_10[] = {2391, 2437};
kal_int32 g_Q_MAX_POS_50_H_CURRENT[] = {2414, 2422};
kal_int32 g_Q_MAX_POS_25_H_CURRENT[] = {2347, 2429}; //2497 modified by zhufeng 20180131 for discharging too fast
kal_int32 g_Q_MAX_POS_0_H_CURRENT[] = {1740, 2173}; //1790 modified by zhufeng 20180131 for discharging too fast
kal_int32 g_Q_MAX_NEG_10_H_CURRENT[] = {924, 1642};
...
/*END: modified by zhufeng 20171225 for multi battery support*/
...
/*BEGIIN: modified by zhufeng 20171225 for multi battery support*/
	//pin_r_value 10K  100K
	typedef signed int      kal_int32;  
	kal_int32 veken_min =(0.000*1000000);              // 0~0.25V
	kal_int32 veken_max =(0.250*1000000);              
	kal_int32 byd_li_ion_min        =(0.250*1000000);  //0.25~1V
	kal_int32 byd_li_ion_max        =(1.000*1000000);  
...
//	}
	...
	for (id = 0; id < TOTAL_BATTERY_NUMBER; id++) {
		if ((veken_min< id_volt )&&( id_volt < veken_max)) {
			g_fg_battery_id = 0;//veken battery
		  printk("[fgauge_get_profile_id Vbat]battery id   g_fg_battery_id = 0 (veken battery) ****.\n");
			break;
		} else if ((byd_li_ion_min< id_volt )&&( id_volt < byd_li_ion_max)) {
			g_fg_battery_id = 1;//byd battery
			printk("[fgauge_get_profile_id Vbat]battery id   g_fg_battery_id = 1 (byd battery) ****.\n");
		}
	}
/*END: modified by zhufeng 20171225 for multi battery support*/
...
/*BEGIN: Added by zhufeng 20171225 for multi battery support*/
if (g_fg_battery_id)
{
		if (Res >= Batt_Temperature_Table[0].TemperatureR) {
		TBatt_Value = -40;  //add by zhufeng 20180106 for range of extend -30 degree
...
{
	if (Res >= Batt_Veken_Temperature_Table[0].TemperatureR) {
		TBatt_Value = -40;  //add by zhufeng 20180106 for range of extend -30 degree
	} else if (Res <= Batt_Veken_Temperature_Table[20].TemperatureR) {
		TBatt_Value = 60;
	} else {
		RES1 = Batt_Veken_Temperature_Table[0].TemperatureR;
		TMP1 = Batt_Veken_Temperature_Table[0].BatteryTemp;

		for (i = 0; i <= 20; i++) {
			if (Res < Batt_Veken_Temperature_Table[i].TemperatureR) {
				RES1 = Batt_Veken_Temperature_Table[i].TemperatureR;
				TMP1 = Batt_Veken_Temperature_Table[i].BatteryTemp;
			} else {
				RES2 = Batt_Veken_Temperature_Table[i].TemperatureR;
				TMP2 = Batt_Veken_Temperature_Table[i].BatteryTemp;
				break;
			}
		}
...
/*END: Added by zhufeng 20171225 for multi battery support*/	
	return TBatt_Value;
}
...
if (g_fg_battery_id)
	...
	return sizeof(battery_veken_profile_t2) / sizeof(BATTERY_PROFILE_STRUCT);
}
...
if (g_fg_battery_id)
	...
	return sizeof(r_veken_profile_t2) / sizeof(R_PROFILE_STRUCT);
}
...
{
	if (temperature == batt_meter_cust_data.temperature_t0)
		return &battery_veken_profile_t0[0];

	if (temperature == batt_meter_cust_data.temperature_t1)
		return &battery_veken_profile_t1[0];

	if (temperature == batt_meter_cust_data.temperature_t2)
		return &battery_veken_profile_t2[0];

	if (temperature == batt_meter_cust_data.temperature_t3)
		return &battery_veken_profile_t3[0];
...
/*BEGIN: Added by zhufeng 20171225 for multi battery support*/
	...
	{
	if (temperature == batt_meter_cust_data.temperature_t0)
		return &r_veken_profile_t0[0];

	if (temperature == batt_meter_cust_data.temperature_t1)
		return &r_veken_profile_t1[0];

	if (temperature == batt_meter_cust_data.temperature_t2)
		return &r_veken_profile_t2[0];

	if (temperature == batt_meter_cust_data.temperature_t3)
		return &r_veken_profile_t3[0];
...
#if 0// MTK_MULTI_BAT_PROFILE_SUPPORT modified by zhufeng for multi battery support
...
/*BEGIN: Added by zhufeng, 20171221 add battery id node*/		
extern int IMM_GetOneChannelValue_Cali(int Channel, int *voltage);
//#define BATTERY_ID_CHANNEL_NUM 1  

static ssize_t show_FG_g_fg_dbg_profile_battery_id(struct device *dev,
						    struct device_attribute *attr, char *buf)
{
	int FG_g_fg_dbg_profile_battery_id = 0;
	int ret = 0;
		
	ret = IMM_GetOneChannelValue_Cali(BATTERY_ID_CHANNEL_NUM, &FG_g_fg_dbg_profile_battery_id);
	if (ret != 0)
		bm_print(BM_LOG_CRTI, "[fgauge_get_profile_id]FG_g_fg_dbg_profile_battery_id read fail\n");
	else
		bm_print(BM_LOG_CRTI, "[fgauge_get_profile_id]FG_g_fg_dbg_profile_battery_id = %d\n", FG_g_fg_dbg_profile_battery_id);
	
	bm_print(BM_LOG_CRTI, "[FG] FG_g_fg_dbg_profile_battery_id : %d\n",
		 FG_g_fg_dbg_profile_battery_id);
		 
	return sprintf(buf, "%d\n", FG_g_fg_dbg_profile_battery_id);
}

static DEVICE_ATTR(FG_g_fg_dbg_profile_battery_id, 0664, show_FG_g_fg_dbg_profile_battery_id, NULL);

/*END: Added by zhufeng, 20171221 add battery id node*/			   
...
static int battery_meter_probe(struct platform_device *dev)
{
	...
	/*BEGIN: Added by zhufeng, 20171221 add battery id node*/	
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_FG_g_fg_dbg_profile_battery_id);
	/*END: Added by zhufeng, 20171221 add battery id node*/	
