...
//Added by zhufeng, 20171213 add battery 4.4V voltage support			
		if (batt_cust_data.high_battery_4400_voltage_support)
...
static void pchr_turn_on_charging(void)
{
	...
	/*BEGIN: Added by zhufeng 20171215 for support meta mode charge TCL_W2020_D01-47
*/	
	} else if ((g_platform_boot_mode == META_BOOT)) {
		battery_log(BAT_LOG_CRTI,
			    "[BATTERY] In meta mode, enable charging.\n");
		charging_enable = KAL_TRUE;
	} else if (g_platform_boot_mode == ADVMETA_BOOT) {
		battery_log(BAT_LOG_CRTI,
			    "[BATTERY] In advanced meta mode, disable charging.\n");
		charging_enable = KAL_FALSE;
	/*END: Added by zhufeng 20171215 for support meta mode charge TCL_W2020_D01-47
*/		
	} else {
		/*HW initialization */
