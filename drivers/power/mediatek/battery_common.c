				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_BatteryAverageCurrent:
		/*  add 2017.12.19 by wangyibo.
		 *  BAT_BatteryAverageCurrent is updated at function bat_thread_kthread, is need 10s. So at factory test mode, when inset usb,
		 *  thie status cannot update fast, must wait for 10s.
		 */
		data->BAT_BatteryAverageCurrent = BMT_status.ICharging;
	...
	case POWER_SUPPLY_PROP_ChargerVoltage:
		 ...
		 *  thie status cannot update fast, must wait for 10s.
		 */
		data->BAT_ChargerVoltage = BMT_status.charger_vol;
...
static void mt_battery_thermal_check(void)
{
	if ((g_battery_thermal_throttling_flag == 1) || (g_battery_thermal_throttling_flag == 3)) {
		...
		/* ignore default rule */
#else
		if (BMT_status.temperature >= 65) {   //modified by zhufeng 20180108 for increase high temperature 65 BUG: TCL_W2020_D01-8
#if defined(CONFIG_POWER_EXT)
			battery_log(BAT_LOG_CRTI,
...
/*BEGIN: Added by zhufeng, 20171213 add battery 4.4V voltage support*/		
	...
	batt_cust_data.high_battery_4400_voltage_support=0;
...
/*END: Added by zhufeng, 20171213 add battery 4.4V voltage support*/	
