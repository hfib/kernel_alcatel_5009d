...
void fts_update_fw_vendor_id(struct i2c_client *client);
void fts_update_fw_ver(struct i2c_client *client);
...
//add for tp info begin by swb 20171207
    fts_update_fw_ver(client);
    fts_update_fw_vendor_id(client);
#ifdef SUPPORT_READ_TP_VERSION	
    memset(fw_version, 0, sizeof(fw_version));
    sprintf(fw_version, "[FW]0x%02x, [IC]FT6336U",fw_ver[0]);
 ...
 void fts_update_fw_vendor_id(struct i2c_client *client)
{
	...
	reg_addr = 0xA8;
	err = fts_i2c_read_reg(client,reg_addr,&fw_vendor_id);
	if (err < 0)
 ...
 void fts_update_fw_ver(struct i2c_client *client)
{
	...
	reg_addr = 0xA6;
	err = fts_i2c_read_reg(client,reg_addr,&fw_ver[0]);
	if (err < 0)
		dev_err(&client->dev, "fw major version read failed");

	reg_addr = 0xB2;
	err = fts_i2c_read_reg(client,reg_addr,&fw_ver[1]);
	if (err < 0)
		dev_err(&client->dev, "fw minor version read failed");

	reg_addr = 0xB3;
	err = fts_i2c_read_reg(client,reg_addr,&fw_ver[2]);
	if (err < 0)
