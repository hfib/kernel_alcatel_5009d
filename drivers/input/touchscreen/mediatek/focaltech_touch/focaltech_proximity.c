...
*****************************************************************************/
/*fix TP sometime no work begin by swb 20180205*/
#if FTS_PSENSOR_EN
int suspend_power_mode = 0;
#endif
/*fix TP sometime no work end by swb 20180205*/
...
/*fix power off Unplug headset  proximity not work begin by swb 20171219*/
int fts_enter_normal_suspend = 0;
static int fts_proximity_status = 0;
/*fix power off Unplug headset  proximity not work end by swb 20171219*/
...
/*fix power off Unplug headset  proximity not work begin by swb 20171219*/
    if(fts_enter_normal_suspend ){//fix TP sometime no work by swb 20180205
       FTS_DEBUG("swb.[PROXIMITY]fts already enter suspend,Need wait wakeup\n"); 
       return ret;
    }
/*fix power off Unplug headset  proximity not work end by swb 20171219*/    
...
int fts_proximity_recovery(struct i2c_client *client)
{
    ...
    /*fix power off Unplug headset  proximity not work begin by swb 20171219*/
    if(fts_proximity_status == 1 )//fix TP sometime no work by swb 20180205
        ret = fts_enter_proximity_mode(client, 1);
    /*fix power off Unplug headset  proximity not work end by swb 20171219*/

    /*fix TP sometime no work begin by swb 20180205*/
    if(fts_proximity_status == 0 && fts_proximity_data.mode)
        ret = fts_enter_proximity_mode(client, 0);
    /*fix TP sometime no work end by swb 20180205*/
    ...
    switch (command)
    {
        ...
        case SENSOR_ENABLE:
            ...
            else
            {
               ...
               /*fix power off Unplug headset  proximity not work begin by swb 20171219*/
                if(value)
                    fts_proximity_status = 1;
                else
                    fts_proximity_status = 0;
                /*fix power off Unplug headset  proximity not work end by swb 20171219*/
                /* Enable proximity */
                err = fts_enter_proximity_mode(fts_i2c_client, value);
                FTS_DEBUG("swb.[PROXIMITY]SENSOR_ENABLE err = %d", err);
                if(err > 0)
                   err = 0;
            }
            break;
        ...
        case SENSOR_GET_DATA:
            ...
            else
            {
                ...
                if((int)fts_proximity_data.detect)
                    sensor_data->values[0] = 10;//(int)fts_proximity_data.detect;
                else
...
int fts_proximity_readdata(struct i2c_client *client)
{
    ...
    fts_i2c_read_reg(client, FTS_REG_PSENSOR_STATUS, &regvalue);
    
    FTS_DEBUG("swb.[PROXIMITY] p-sensor i2c read reg = %x\n",regvalue);
    ...
    else{//shenwenbin add begin 20171222 for sleep in Pull off the headset ,then powerkey wakeup flicker screen 
        FTS_DEBUG("swb.[PROXIMITY] p-sensor i2c read status error,waiting...\n");
        return 0;
    }
    //shenwenbin add end 20171222 for sleep in Pull off the headset ,then powerkey wakeup flicker screen
    ...
    if (proximity_status != (int)fts_proximity_data.detect)
    {
        ...
        if(proximity_status)
           sensor_data.values[0] = 10;//proximity_status;
        else
           sensor_data.values[0] = 0;    
        sensor_data.value_divide = 1;
        sensor_data.status = SENSOR_STATUS_ACCURACY_MEDIUM;
...
int fts_proximity_suspend(void)
{
    if (fts_proximity_data.mode == 1){
        suspend_power_mode = 1;
        return 0;
    }
    else{
        suspend_power_mode = 0;
        return -1;
        }
}
...
int fts_proximity_init(struct i2c_client *client)
{
    ...
    FTS_ERROR("swb.[PROXIMITY]fts proximity init\n");
    ...
    if (err)
        FTS_ERROR("swb.[PROXIMITY]fts proximity attach fail = %d!", err);
    else
        FTS_ERROR("swb.[PROXIMITY]fts proximity attach ok = %d\n", err);
