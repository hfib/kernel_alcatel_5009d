...
//begin xiaopei add LCM info 
extern char lcm_ver_show_str[64];
//End 
...
//begin xiaopei add LCM info 
static ssize_t lcm_proc_lcm_info_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
    int cnt=0;
    char *page = NULL;
	page = kzalloc(128, GFP_KERNEL);
    if( 0 == strlen(lcm_ver_show_str) )
        cnt = sprintf(page, "no LCM!\n");
	else
	{
		cnt = sprintf(page, "[LCM]%s\n", lcm_ver_show_str );
	}
	cnt = simple_read_from_buffer(buf, size, ppos, page, cnt);	
	kfree(page);
    return cnt;
...
static const struct file_operations lcm_proc_lcm_info_fops = {
	.read		= lcm_proc_lcm_info_read,
};
//end add 12-14
...
static int tp_fm_creat_proc_entry(void)
{
	...
	//Begin xiaopei add LCM INFO 
	struct proc_dir_entry *proc_entry_lcm;
    //End add LCM info 
    ...
    //Begin xiaopei add LCM INFO
	proc_entry_lcm = proc_create_data("lcd_info", 0444, NULL, &lcm_proc_lcm_info_fops, NULL);
	if (IS_ERR_OR_NULL(proc_entry_lcm))
	{
		pr_err("add /proc/lcm_info error \n");
	}
	//End 12-14
