...
//add for tp info begin by swb 20171207    
#include "../lct_tp_fm_info.h"
//add for tp info end by swb 20171207    
...
static int Ilitek_proximity_want_status = 0;
static int Ilitek_enter_normal_suspend = 0;
...
s32 DrvEnableProximity(void)
{
    ...
    szTxData[3] = 0xa0;
/*fix power off Unplug headset  proximity not work begin by swb 20171219*/    
    if(Ilitek_enter_normal_suspend){
       DBG(&g_I2cClient->dev,"swb.[PROXIMITY]Ilitek already enter suspend,Need wait wakeup\n"); 
       return 0;
    }
/*fix power off Unplug headset  proximity not work end by swb 20171219*/
...
/*fix TP sometime reusme not work begin by swb 20171229*/    
     if(Ilitek_enter_normal_suspend){
         DBG(&g_I2cClient->dev,"swb.[PROXIMITY]Ilitek already enter suspend,Need wait wakeup\n"); 
         return 0;
        }
/*fix fix TP sometime reusme not work end by swb 20171229*/
...
int DrvGetTpPsData(void)
{
    ...
    if(0 == g_FaceClosingTp)
        return 0;       
    else if(1 == g_FaceClosingTp)
        return 10;
    else
        return g_FaceClosingTp;
}
    ...
    switch (nCommand)
    {
        ...
        case SENSOR_ENABLE:
            ...
            else
            {
                ...
                {
/*fix power off Unplug headset  proximity not work begin by swb 20171219*/
                    Ilitek_proximity_want_status = 1;
/*fix power off Unplug headset  proximity not work end by swb 20171219*/
                ...
                {
/*fix power off Unplug headset  proximity not work begin by swb 20171219*/                
                    Ilitek_proximity_want_status = 0;
/*fix power off Unplug headset  proximity not work end by swb 20171219*/
...
/*fix power off Unplug headset  proximity not work begin by swb 20171219*/
static void _DrvProximityRecovery(void)
{
    DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__); 
//modify 20171229 by shenwenbin for resume cover proximity
    if (Ilitek_proximity_want_status == 1)
    {
        if (DrvEnableProximity() < 0)
        {
            DBG(&g_I2cClient->dev, "Enable ps fail\n"); 
            return;
        }
    }else{/*fix TP sometime reusme not work begin by swb 20171229*/
        if (g_EnableTpProximity){
            if (DrvDisableProximity() < 0)
            {
                DBG(&g_I2cClient->dev, "Disable ps fail\n"); 
                return;
            }
        }
    } 
/*fix TP sometime reusme not work end by swb 20171229*/
    return;
}
/*fix power off Unplug headset  proximity not work end by swb 20171219*/
#endif
...
#endif //CONFIG_PLATFORM_USE_ANDROID_SDK_6_UPWARD
{
    ...
    {
        ps_flag = 0;
        DBG(&g_I2cClient->dev, "g_EnableTpProximity = %d;ps_flag = %d\n",g_EnableTpProximity,ps_flag);
        return;
    }
#endif //CONFIG_ENABLE_PROXIMITY_DETECTION
...
//add for tp info begin by swb 20171207  
    ...
    sprintf(fw_version, "[FW]0x%02x,0x%02x [IC]MSG2238",nMajor,nMinor);
