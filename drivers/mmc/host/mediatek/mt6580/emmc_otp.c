...
unsigned int emmc_get_wp_size(void)
{
	...
	if (0 != sg_wp_size) {
		//Longcheer modify for tcl FR,save IMEI in OTP partion. by wanjie@longcheer.com at 2017/12/15  start
		//pr_debug("msdc[%s:%d]mmc_host = 0x%p,sg_wp_size=%d\n", __func__,
		//	__LINE__, host_ctl->mmc, sg_wp_size);
		//Longcheer modify for tcl FR,save IMEI in OTP partion. by wanjie@longcheer.com at 2017/12/15  end
		return sg_wp_size;
	}
