...
u32 pad_macro;
static void enable_tee_clk(struct mt_spi_t *ms)
{
...
{
	enable_tee_clk(spi_master_get_devdata(ms->master));
}

static void disable_tee_clk(struct mt_spi_t *ms)
{
...
{
	disable_tee_clk(spi_master_get_devdata(ms->master));
}
#ifdef SPI_DEBUG
